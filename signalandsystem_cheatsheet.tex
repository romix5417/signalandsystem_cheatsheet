\documentclass[10pt,landscape,UTF8]{article}
\usepackage{multicol}
\usepackage{calc}
\usepackage{CTEX}
\usepackage{ifthen}
...
\usepackage[pdftex,
            ...
            pdftitle={Signal and System Cheatsheet},
...
\begin{center}
    {\color{blue} \Large{\textbf{信号与系统备忘}}} \\
...
\section{信号与系统基本概念}\smallskip \hrule height 2pt \smallskip

\subsection{信号能量与功率} 
    连续时间信号$x(t)$在$t1 \leq t \leq t2$内的总能量可以定义为:
    \begin{minipage}{\linewidth}
        \begin{equation*}
            \int_{t2}^{t1} \left | x(t) \right |^2dt
        \end{equation*}            
    \end{minipage}

    离散时间信号$x[t]$在$n1 \leq n \leq n2$内的总能量可以定义为:
    \begin{minipage}{\linewidth}
        \begin{equation*}
            \sum_{n2}^{n1}\left | x[n] \right |^2
        \end{equation*}            
    \end{minipage}
...
\subsection{自变量变换}
    ...
    周期信号: $x(t)=x(t+T)$ and $x[n]=x[n+N]$

    偶信号: $x(-t)=x(t)$ and $x[-n]=x[n]$

    奇信号: $x(-t)=-x(t)$ and $x[-n]=-x[n]$

    重要结论: 
    
    任何信号都可以分解成为连个信号之和，一个偶信号，另一个为奇信号。
...
\subsection{指数信号与正弦信号}  
美妙的欧拉公式: $e^{iw_0t}=cos(w_0t)+isin(w_0t)$

复指数信号: $x(t)=Ce^{at}$, $x(t)=e^{jw_0t}$ and $x(n)=e^{jw_0n}$

正弦信号: $Acos(w_0t+\phi)$ and $Acos(w_0n+\phi)$

正弦信号使用复指数信号表示: 
$Acos(w_0t+\phi)=\frac{A}{2}e^{j\phi}e^{jw_0t}+
\frac{A}{2}e^{-j\phi}e^{-jw_0t}$

$Acos(w_0n+\phi)=\frac{A}{2}e^{j\phi}e^{jw_0n}+
\frac{A}{2}e^{-j\phi}e^{-jw_0n}$

$e^{jw_0t}$和$e^{jw_0n}$信号的比较:
\begin{minipage}{\linewidth}
    \begin{flushleft}
    \begin{tabular}{p{3.97cm}<\centering p{3.97cm}<\centering}
        \hline
        $e^{jw_0t}$& $e^{jw_0n}$\\
        \hline
        $w_0$不同，信号不同     & 频率相差$2\pi$整数倍，信号相同\\
        对任何$w_0$值都是周期的 & 仅当$w_0=2\pi m/N$时才是周期的，
                                这里$N(>0)$和m均为整数\\
        基波频率为$w_0$         & 基波频率$w_0/m$\\
        基波周期：$w_0=0$,无定义 $w_0\neq 0$,$2\pi/w_0$ & 
        基波周期：$w_0=0$,无定义$w_0\neq 0$,$m(\frac{2\pi}{{w_0n}})$ \\
        \hline
    \end{tabular}
    \end{flushleft}
\end{minipage}
...
\draw [-] (-3.0,0)--(3.0,0);
\draw [-] (0,0)--(0,1);
\draw [fill] (0,1) circle [radius = 0.1] node [above right] {$\delta[n]$};
\draw [fill] (0.5,0) circle [radius = 0.1];
\draw [fill] (1.0,0) circle [radius = 0.1];
\draw [fill] (1.5,0) circle [radius = 0.1];
\draw [fill] (2.0,0) circle [radius = 0.1];
\draw [fill] (-0.5,0) circle [radius = 0.1];
\draw [fill] (-1.0,0) circle [radius = 0.1];
\draw [fill] (-1.5,0) circle [radius = 0.1];
\draw [fill] (-2.0,0) circle [radius = 0.1];
\draw node (0,0) [below] {0};
\end{tikzpicture}
\end{minipage}
...
\draw [fill] (0.5,1) circle [radius = 0.1];
\draw [fill] (1.0,1) circle [radius = 0.1] node [above] {$u[n]$};
\draw [fill] (1.5,1) circle [radius = 0.1];
\draw [fill] (2.0,1) circle [radius = 0.1];
\draw [fill] (-0.5,0) circle [radius = 0.1];
\draw [fill] (-1.0,0) circle [radius = 0.1];
\draw [fill] (-1.5,0) circle [radius = 0.1];
\draw [fill] (-2.0,0) circle [radius = 0.1];
...
\draw [fill] (2.6,0.5) circle [radius = 0.05];
\draw [fill] (2.9,0.5) circle [radius = 0.05];

\draw node (0,0) [below] {0};
\end{tikzpicture}
\end{minipage}

重要结论：离散时间单位脉冲是离散时间单位阶跃的一次差分。
...
\centering
\begin{tikzpicture}[xscale=1.0]
\draw [-] (-3.0,0)--(3.0,0) node [below] {t};
\draw [-] [line width=1] (-3.0,0)--(0,0);
\draw [-] [line width=1] (0,0)--(0,1);
\draw [-] [line width=1] (0,1)--(3,1);
\draw [->] (0,0)--(0,2) node [above right] {$u(t)$};
\draw node (0,0) [below] {0};
\end{tikzpicture}
\end{minipage}

重要结论:连续阶跃函数是连续单位冲激的积分函数。
...
\begin{itemize}
    ...
    无记忆系统:系统的输出仅仅决定于该时刻的输入

    记忆系统:当前的输入与输入和输出的将来值有关系
    \item 可逆性与可逆系统
    可逆系统:系统如果在不同的输入下，导致不同的输出，则称其为可逆系统。
    \item 因果性
    因果系统:如果一个系统再任何时刻的输出只决定于现在和过去的输入，那么称其为因果系统。
    \item 稳定性
    稳定系统:响应不会无界增长的系统。
    \item 时不变性
    时不变系统:系统的特性行为不随着时间而变，则称其为时不变系统。
    \item 线性
    ...
    (2)$ay_1(t)$是对$ax_1(t)$的响应，此处a为任意复常数。
\end{itemize}


\section{线性时不变系统} \smallskip \hrule height 2pt \smallskip

\subsection{离散时间线性时不变系统:卷积和}
一个离散时间信号可以用一组幅度加权和延时的单位样本序列的和来表示。
\begin{equation*}
x[n]=\sum_{k=-\infty}^{+\infty}x[k]\delta[n-k]
\end{equation*}
若x[n]=u[n],即单位阶跃序列，在这种情况下$k<0$,时$u[k]=0$，而$k\geq0$时$u[k]=1$，则式
子可以表示为：
\begin{equation*}
u[n]=\sum_{k=0}^{+\infty}\delta[n-k]
\end{equation*}
根据线性系统的叠加性质，$x[n]$对应的响应为$y[n]$，$h[n-k]$为单位脉冲序列响应，那么则可
以得到卷积和:
\begin{equation*}    
y[n]=\sum_{k=-\infty}^{+\infty}x[k]h[n-k]
\end{equation*}
重要结论:既然一个线性时不变系统对任意输入的响应可以用系统对单位脉冲的响应来表示，那
么，线性时不变系统的单位脉冲响应就完全刻画了系统的特征。

    
\subsection{连续时间线性时不变系统:卷积积分}
    \begin{description}
...
\section{周期信号的傅立叶级数表示}\smallskip \hrule height 2pt \smallskip

    
\subsection{线性时不变系统对复指数信号的响应}
\begin{description}
...
\subsection{连续时间周期信号的傅立叶级数表示}

\subsection{傅立叶级数的收敛}

\subsection{连续时间傅立叶级数性质}

\subsection{离散时间周期信号的傅立叶级数表示}

\subsection{离散时间傅立叶级数性质}

\subsection{傅立叶级数与线性时不变系统}

\subsection{滤波}

\section{连续时间傅立叶变换}\smallskip \hrule height 2pt \smallskip


\subsection{非周期信号的表示:连续时间傅立叶变换}
\begin{description}
...
\subsection{由线性常系数微分方程表征的系统}


\section{离散时间傅立叶变换}\smallskip \hrule height 2pt \smallskip

\subsection{非周期信号的表示:离散时间傅立叶变换}
\begin{description}
...
\subsection{离散时间傅立叶变换性质} When you plug any CRV into its own CDF, you get a Uniform(0,1) random variable. When you plug a Uniform(0,1) r.v.~into an inverse CDF, you get an r.v.~with that CDF. For example, let's say that a random variable $X$ has CDF
...
%    \end{align*}
%    
%    Intuitively, we would expect that $T$ is distributed Geometrically since $T$ is just a filtered version of $N$, which itself is Geometrically distributed. The MGF of $X\sim\Geom(\phi)$ is \[E(e^{tX}) = \displaystyle\frac{\phi}{1-(1-\phi) e^t}\]
%    
%    So, we would want to try to get our MGF into this form to identify what $\phi$ is. Taking our original MGF, it would appear that dividing by $s+(1-s)p$ would allow us to do this. Therefore, we have that \begin{center}
%    $E(e^{tT}) = \displaystyle\frac{s}{s+(1-s)p - (1-s)pe^t} = \frac{\frac{s}{s+(1-s)p}}{1-\frac{(1-s)p}{s+(1-s)p}e^t}$
%    \end{center}
%    
%    By pattern-matching, it thus follows that $\boxed{T \sim \Geom(\phi)}$ where \[\boxed{\phi = \displaystyle\frac{s}{s+(1-s)p}}\]
...
\hline
\shortstack{Log-Normal \\ $\mathcal{LN}(\mu,\sigma^2)$} & \shortstack{$\frac{1}{x\sigma \sqrt{2\pi}}e^{-(\log x - \mu)^2/(2\sigma^2)}$\\$x \in (0, \infty)$} & $\phi = e^{ \mu + \sigma^2/2}$ & $\phi^2 (e^{\sigma^2} - 1)$ & doesn't exist\\
\hline
